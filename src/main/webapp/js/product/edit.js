...
$(function () {
  ...
  $("#btn-insert").on("click", function (e) {
    ...
    const mainImageFile = $("#mainImage")[0].files; // 새로 선택한 파일
      // 화면에 <img> 태그가 보이고 있고, src에 데이터가 있는지 확인
      const isExistMainImg = $(".inputImg.main img").is(":visible") && $(".inputImg.main img").attr("src") !== "";

      if (!isExistMainImg && (!mainImageFile || mainImageFile.length === 0)) {
        alert("代表画像を追加してください。");
        return;
      }
    $("#productForm").submit();
